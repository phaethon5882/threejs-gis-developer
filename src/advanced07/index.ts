...
class App {
  ...
  private readonly createTable = () => {
    const geometry = new THREE.BoxGeometry();
    const material = new THREE.MeshPhongMaterial({ color: '#878787' });
    const table = new THREE.Mesh(geometry, material);

    table.position.set(0, -0.525, 0);
    table.scale.set(30, 0.5, 30);
    table.receiveShadow = true;
    this.scene.add(table);
  };

  private readonly createDomino = () => {
    // 1. 도미노가 배치될 Path
  };
