import type { Scene, WebGLRenderer, Object3D, Camera, OrthographicCamera } from 'three';
import { PerspectiveCamera, DirectionalLight, DirectionalLightHelper } from 'three';
...
const CAMERA_TARGET_PIVOT = 'targetPivot';
const ORTHOGRAPHIC_MULT = 10;
...
class App {
  ...
  constructor(container: HTMLDivElement) {
    ...
    window.onresize = debounce(this.resize, 250);
    this.resize();

    requestAnimationFrame(this.render);
  }

  private readonly setupCamera = (): Camera => {
  ...
  private readonly resize = (): void => {
    ...
    if (camera instanceof PerspectiveCamera) {
      ...
      camera.left = -ORTHOGRAPHIC_MULT * aspect;
      camera.right = ORTHOGRAPHIC_MULT * aspect;
    }
  ...
  private readonly update = (time: DOMHighResTimeStamp): void => {
    ...
    if (smallSpherePivot != null) {
      ...
      if (this.light instanceof DirectionalLight) {
        ...
        const targetPivot = this.scene.getObjectByName(CAMERA_TARGET_PIVOT);
        if (targetPivot != null) {
